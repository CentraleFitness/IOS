...
class ViewRegistration: UIViewController, UITextFieldDelegate {
    ...
    @IBAction func registration(sender : UIButton)
    {
        ...
        {
            if (isPasswordValid(_password.text!) == true)
            {
        let parameters: Parameters = [
            ...
            "phone number": "0624891343"
                ...
                Alamofire.request("\(network.ipAdress.rawValue)/registration", method: .post, parameters: parameters, encoding: JSONEncoding.default)
            .responseJSON { response in
                if let json = response.result.value as? [String: Any] {
                    ...
                    {
                        self.alert("Félicitation", message: "Votre compte a bien été créer")
                        // let login = json["token"] as? String
                        //  print(login!)
                    }
                }
