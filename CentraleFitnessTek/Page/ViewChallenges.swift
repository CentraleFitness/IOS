...
class ViewChallenges: UIViewController {
    ...
    func createArray_2() -> [Challenges] {
        ...
        Alamofire.request("http://163.5.84.201:8080/get/events", method: .post, parameters: parameters, encoding: JSONEncoding.default)
            .responseJSON { response in
                if let json = response.result.value as? [String: Any] {
                    ...
                    if (error == "false")
                    {
                        ...
                        self.list_events += Event.getEventArray(array: test)
                        self.fill_event()
                        print()
                    }
                }
    ...
    func getEventPreview(id: Int, event: Event, isSuccess: @escaping(_ event: Event,_ id: Int)-> Void){
        ...
        Alamofire.request("http://163.5.84.201:8080/get/eventpreview", method: .post, parameters: parameters, encoding: JSONEncoding.default)
            .responseJSON { response in
                if let json = response.result.value as? [String: Any] {
    ...
    func showStandardDialog(animated: Bool = true) {
        ...
        // Create first button
        let buttonOne = CancelButton(title: "Annulé") {
    ...
    func subscribe_event(){
        ...
        Alamofire.request("http://163.5.84.201:8080/event/registration", method: .post, parameters: parameters, encoding: JSONEncoding.default)
            .responseJSON { response in
                if let json = response.result.value as? [String: Any] {
                    let error = json["error"] as? String
                    print(error!)
